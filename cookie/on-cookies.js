import { encodeData, decodeData, getUserType, setCookie, generateToken, getMetaTags, updateCookiePreferences } from "./utils.js";
import languageDictionary from "./language.dictionary.js";
import { DAY, COOKIE_TYPES } from "./constants.js";
...
document.addEventListener("DOMContentLoaded", function () {
     ...
     * 
     * //* ВАЖЛИВО: можна не вказувати banner, видаливши рядок всередині new OnCookies: banner: "шлях/до/баннеру.png".
     * 
...
class OnCookies {
    ...
    createBanner() {
        ...
        dialog.className = 'ont-rounded-xl ont-border ont-border-neutral-300 ont-overflow-hidden ont-flex ont-flex-col ont-items-center ont-justify-center ont-gap-4 ont-max-w-[20rem] sm:ont-max-w-[30rem] ont-shadow-2xl ont-opacity-0 ont-transition-opacity ont-duration-700 ont-hidden';
        ...
        const essentialCookies = this.createCookieCategory(texts["cookie-settings__subheading1"], texts["cookie-settings__description1"], 1);
        cookieSettings.appendChild(essentialCookies);

        const performanceCookies = this.createCookieCategory(texts["cookie-settings__subheading2"], texts["cookie-settings__description2"], 2);
        cookieSettings.appendChild(performanceCookies);

        const advertisingCookies = this.createCookieCategory( texts["cookie-settings__subheading3"], texts["cookie-settings__description3"], 3);
        ...
        savePreferencesButton.className = 'ont-flex ont-w-fit ont-items-start ont-p-2 ont-font-medium ont-transition-all ont-duration-300 primary-color ont-text-white ont-rounded-xl hover:ont-brightness-75';
        ...
        backToBanner.className = 'ont-text-sm ont-text-neutral-500 ont-font-medium hover:ont-text-neutral-700 ont-transition-all ont-duration-300 ont-cursor-pointer ont-tracking-wider ont-w-fit ont-bg-white';
    ...
    createCookieIcon() {
        ...
        cookieIcon.className = "ont-flex ont-hidden ont-items-center ont-justify-center ont-h-12 ont-w-12 ont-fixed ont-bottom-[3%] ont-left-[3%] ont-rounded-full ont-bg-white ont-border ont-border-slate-300 hover:ont-bg-blue-200 ont-group hover:primary-color ont-transition-all ont-duration-300 ont-z-[9999] ont-cursor-pointer";
